...
if cmd_args.multi_hdu:
    ...
    outprefix = "{fitsfile}-cleaned-{method}{output_id}".format(
        **vars(cmd_args))
...
def save_aux_image(image, label):
    ...
    If in multi-HDU mode, just append it to the list of HDUs
    to be written later.
    ...
    if cmd_args.multi_hdu:
        ...
        pyfits.PrimaryHDU(image).writeto(
            outprefix + "-" + label + ".fits", clobber=True)
...
if cmd_args.method == "thresh":
    if cmd_args.threshold is None:
        raise ValueError(
            "The --threshold argument is obligatory with --method thresh")
    ...
    if cmd_args.method == "edge":
        ...
        badpix = dilation(erosion(badpix.astype(np.uint8), cross3),
                          square3).astype(bool)
